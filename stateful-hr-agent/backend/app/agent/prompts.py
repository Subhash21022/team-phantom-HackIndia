...
6. DB ARBITRARY FILTERS: If the user query implies filtering DB records (e.g. "show engineering candidates", "list employees in sales", "who is hired"), pass ANY extracted filter parameters (e.g., `{"department": "sales"}`, `{"role": "engineering"}`, `{"status": "hired"}`) directly into the `parameters` of `get_candidates` or `get_employees`. The DB MCP will map them dynamically.
